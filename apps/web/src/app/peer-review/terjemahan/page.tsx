    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="mt-6 rounded-xl border border-[var(--line)] bg-[var(--panel)] p-5 text-xs text-[var(--muted)] leading-relaxed space-y-2">
        <div className="font-bold text-white flex items-center gap-2">
          <span>🌐</span>
          Bahasa Sasaran: <span className="text-sky-400">{langNames[selectedLang]}</span>
        </div>
        <p>
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2 border-b border-[var(--line)] pb-3">
                <div>
                  <span className="rounded bg-slate-800 text-slate-400 font-mono text-[10px] px-2 py-0.5 uppercase">
                    {dim.id}
                  </span>
                  <h3 className="font-bold text-base text-white/95 mt-1">{dim.name_id}</h3>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-emerald-500/20 text-emerald-400 px-2.5 py-0.5 text-[10px] font-semibold">
                    ✓ Lolos QA Tim
                  </span>
                  <button
                    onClick={() =>
                    ...
                    className="rounded-lg border border-[var(--line)] bg-[var(--bg)] px-3 py-1 text-xs font-semibold text-sky-400 hover:border-sky-500 transition"
                  >
                    ✏️ Usulkan Koreksi
                  </button>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 text-xs">
                ...
                <div className="rounded-lg bg-[var(--bg)] p-3.5 border border-[var(--line)] space-y-1.5">
                  ...
                  <p className="font-semibold text-white/90">{dim.name_id}</p>
                ...
                <div className="rounded-lg bg-sky-950/20 p-3.5 border border-sky-500/20 space-y-1.5">
                  ...
                  <p className="font-semibold text-sky-200">{trans?.name || "Belum diterjemahkan"}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-6 shadow-2xl space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between border-b border-[var(--line)] pb-3">
              <h3 className="font-bold text-sm text-white">Usulkan Koreksi Terjemahan</h3>
              <button
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSendSuggestion} className="space-y-3 text-xs">
                <div>
                  ...
                  <strong className="text-white">{suggestModal.dimNameId}</strong>
                </div>
                <div>
                  <span className="text-[var(--muted)]">Terjemahan Saat Ini ({selectedLang.toUpperCase()}):</span>{" "}
                  <div className="mt-1 p-2 rounded bg-[var(--bg)] border border-[var(--line)] text-slate-300">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    className="w-full rounded-lg border border-[var(--line)] bg-[var(--bg)] p-2.5 text-white focus:border-sky-500 focus:outline-none"
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    type="button"
                    onClick={() => setSuggestModal(null)}
                    className="flex-1 rounded-lg border border-[var(--line)] py-2 text-[var(--muted)] hover:text-white"
                  >
                    Batal
                  </button>
                  <button
